    ...
    return (
        <div className="p-6 rounded-2xl bg-[var(--bg)] border border-[var(--border)] shadow-sm hover:shadow-md transition-all duration-300 group flex flex-col">
            <div className="flex justify-between items-center mb-6">
                <span className="text-sm font-semibold text-[var(--text)] uppercase tracking-wider">{title}</span>
            ...
            <div className="flex items-baseline gap-1 mb-2">
                {value && (
                    <span className="text-5xl font-bold text-[var(--text-h)] tracking-tight">{value}</span>
                )}
                {suffix && (
